...
class Main:
    ...
    #                 # Khi nhấn Confirm, cập nhật settings và vào game
    #                 selections = self.selection_screen.get_selections()
    #                 # Cập nhật settings
    #                 global choice_background, choice_fish
    #                 choice_background = selections["map"]
    #                 print(choice_background)
    #                 choice_fish = selections["character"]
    #                 update_background()  # Cập nhật images_background
                    
    #                 # Tất cả control đều mặc định là phím lên xuống, nên không cần xử lý control
    #                 self.game = Game()
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if self.selection_screen.handle_click(event.pos, map_rects, control_rects, char_rects):
                    ...
                    choice_background = selections["map"]
                    choice_fish = selections["character"]
                    print(f"Before update_background: choice_background = {choice_background}")
                    update_background()
                    print(f"After update_background: images_background = {images_background}")
                    self.game = Game()
                    return GameState.GAME
            if event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE:
                return GameState.MENU
